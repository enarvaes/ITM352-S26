...
# Local data file for this dashboard.
...
def validate_required_fields(data):
    # Let the user know right away if some columns are missing.
...
def load_csv(file_path):
    ...
    try:
        ...
        # Stop cleanly if the file cannot be loaded.
...
def average_sales_by_region_with_state_sale_type(data):
    ...
    avg_state_sale_type = pd.pivot_table(
        data,
...
def total_sales_quantity_and_price_by_customer_type(data):
    ...
    required = {"customer_type", "quantity", "sales"}
    if not required.issubset(data.columns):
        print("Missing required columns: customer_type, quantity, and/or sales.")
        return
...
def max_min_sales_price_by_category(data):
    ...
    required = {"product_category", "sales"}
    if not required.issubset(data.columns):
        print("Missing required columns: product_category and/or sales.")
        return
...
def display_menu():
    # Menu is kept in one tuple so I can edit choices in one place.
    menu_options = (
...
def main():
    # Show menu over and over until user exits.
